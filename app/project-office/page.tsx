  ...
  return <div className="po-page" id="top">
    ...
    <main>
      ...
      <section className="po-section" id="projects">
        ...
        <div className="po-ecosystem"><div className="po-eco-intro"><p className="po-eyebrow">Зарегистрированные разработки</p><h3>Разработанные программы для ЭВМ</h3><p>Программы созданы Иваном самостоятельно или в соавторстве для анализа экспериментальных данных, организации лабораторной работы и моделирования биологических процессов. Все перечисленные программы зарегистрированы.</p></div>{softwareGroups.map((group,i)=><article className={`po-software g${i+1}`} key={group.title}><span>Программа для ЭВМ</span><h4>{group.title}</h4><p>{group.note}</p><ul>{group.items.map(([name,purpose,number])=><li key={name}><b>{name}</b><span>{purpose}</span><small>Автор или соавтор — Иван Иващенко · регистрация № {number}</small></li>)}</ul></article>)}</div>
      </section>
